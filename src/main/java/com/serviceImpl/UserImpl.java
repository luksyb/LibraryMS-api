...
public class UserImpl implements User {
    ...
    @Override
    public List<Book> getAll(String name, String author, String thumbnailImage) {
        List<Book> books = new ArrayList<Book>();
        for(Book book : Book.globalBookList) {
            if (book.getBookName().equals(name) && book.getBookAuthor().equals(author) && book.getBookThumbnailImage().equals(thumbnailImage)) {
                books.add(book);
            }
        }
        return books;
